/**
 * router-commandcode: task-aware reasoning-mode router for the commandcode
 * provider, with a continuous react↔spec axis.
 *
 * A provider-scoped copy of router-standard's `router-bootstrap`, tuned for
 * commandcode's deepseek/deepseek-v4-flash with three measured adaptations (A/B/C,
 ...
 *     the preset's intended `commandcode` provider.
 *
...
} from './router-commandcode-core-v1.mjs'

/** Cordis plugin name used by loader diagnostics. */
export const name = 'router-commandcode'

/** The provider this preset is intended for (informational; not a hard gate). */
const SCOPE_PROVIDER = 'commandcode'
...
export function apply(ctx, config) {
  ...
  //    已删除；深度引导静态并入 WEAK_FLASH persona（router-commandcode-core-v1.mjs）。
  ...
  registerTool({
    ...
    execute() {
      ...
      if (route.provider !== SCOPE_PROVIDER) {
        lines.push(`note: preset router-commandcode is designed for provider "${SCOPE_PROVIDER}" (session is on ${route.provider ?? '(none)'})`)
      }
      return lines.join('\n')
    },
  })
