#!/usr/bin/env bash
# router-opencode-go 一键健康检查（verify.sh）
...
echo "===== router-opencode-go 健康检查 ($(date '+%F %T')) ====="
...
sys.exit(0 if m and m.group(1)=="deepseek-official" else 1)' 2>/dev/null; then
    ok "默认模型 = deepseek-official（未被 selectModel 污染）"
  else
    bad "默认模型被改为 opencode-go——恢复方法见 OPERATIONS.md §5 坑 2"
  fi
else
