# router-opencode-go 运维手册（OPERATIONS）
...
# ── 第 3 步：安装预设 ─────────────────────────────────────────────
cp -a ~/.dsh/.agent-presets/router-opencode-go ~/.dsh/.agent-presets/router-opencode-go.new
...
#   期望列表含 router-opencode-go（composition 合法；不合法会以 broken 行出现）

# ── 第 5 步：端到端冒烟（新会话）──────────────────────────────────
#   ① GUI 新建会话 → 预设选 Router Opencode-Go (experimental)
#   ② 模型选 opencode-go/deepseek-v4-flash，reasoningEffort 设 max
#   ③ 发一条消息，然后让模型调用 dev_router_status，期望：
#      route=opencode-go/deepseek-v4-flash (source=request)
#      mode=weak (band=weak)
...
```bash
bash ~/.dsh/.agent-presets/router-opencode-go/scripts/verify.sh
```
...
- **方案 A：换文件名（免重启，推荐）**
  1. 修改 `router-go-bootstrap-v2.mjs`（或 `router-go-core.mjs`）→ 保存为**新文件名**
     （如 `router-go-bootstrap-v3.mjs`；core 被 import 时 bootstrap 也要跟着改名）
...
2. **selectModel 会写全局默认模型**：API 调 `session.selectModel` 会触发
   ...
   agent-default-model:
     ...
     reasoningEffort: max
   ```
3. **默认模型勿改**：预设按"手动选模型"设计（用户约束）。默认保持 deepseek-official。
...
| 请求里没看到注入的 system | 预设 | request/header 事件验证；确认会话模型是 opencode-go（`dev_router_status` 的 route 行） |
| 模型无响应/报错 | 网关 | opencode-go key（OPENCODE_GO_API_KEY）有效性；llm-pi-ai 的 request/context 事件 |
...
rm -rf ~/.dsh/.agent-presets/router-opencode-go
...
| `router-go-bootstrap-v2.mjs` | 路由插件：persona 注入/工具裁剪/provider 记录/`sessionModeUser` 过滤 |
| `router-go-core.mjs` | 纯路由逻辑（persona 文本/分类/带宽映射，零依赖） |
