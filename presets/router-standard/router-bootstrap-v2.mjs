...
import {
  ...
  isComplexTask, classifyTask, extractText,
...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    const mode = overrides.get(session.id) ?? sessionModeUser(session)
  ...
  /** 过滤版 sessionMode：跳过 plugin/系统注入消息（sage-mem 等），
   * 只分类真实用户消息——否则首条注入的记忆文本关键词会把模式带偏
   * （实测 sage-mem 文本把 "你好" 会话污染成 spec）。 */
  ...
  registerTool({
    ...
    execute(args) {
      ...
      const current = overrides.get(session.id) ?? sessionModeUser(session)
